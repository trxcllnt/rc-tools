...
gulp.task('react-native-init', (done) => {
  ...
  const bundleBasePath = path.join(__dirname, '../react-native-boilerplates/');
  targz().extract(path.join(bundleBasePath, `${bundleName}.tar.gz`), bundleBasePath)
  .then(() => {
    fs.copySync(path.join(__dirname, `../react-native-boilerplates/${bundleName}/ios`),
      resolveCwd('ios'));
    fs.copySync(path.join(__dirname, `../react-native-boilerplates/${bundleName}/android`),
      resolveCwd('android'));
    done();
  });
});
